...
const AdminMetricsPage: React.FC = () => {
  ...
  const fetchMetrics = async (window?: {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setError({
          code: res.error?.code,
          message: res.error?.message ?? "Failed to load metrics",
        });
      }
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : "Network error loading metrics";
  ...
  useEffect(() => {
    void fetchMetrics(growthWindow);
  }, [growthWindow]);
  ...
  if (loading && !data) {
    return (
      ...
      >
        <RefreshCw
          size={32}
          style={{ animation: "spin 1s linear infinite", color: "#C8F032" }}
        />
    ...
    >
      ...
      >
        <div>
          ...
          >
            ...
            >
              <BarChart3 size={20} color="#0A1E28" />
            </div>
            <h1
              style={{
                margin: 0,
          ...
          {[
            {
              ...
              value: data?.users.total_registered,
              sub: "All time",
              icon: <Users size={15} />,
            },
            {
              ...
              sub: "KYC-verified & not suspended",
              icon: <CheckCircle size={15} />,
            },
            {
              label: "Suspended",
              value: data?.users.total_suspended,
              sub: null,
              icon: <AlertCircle size={15} />,
            },
            {
              label: "At Launch",
              value: data?.users.users_at_launch,
              sub: null,
              icon: <Activity size={15} />,
            },
            {
              label: "Monthly Active",
              value: data?.users.monthly_active_users,
              sub: "Last 30 days · live",
              icon: <Activity size={15} />,
            },
          ].map((stat, i, arr) => (
            <div
      ...
      <motion.div {...card(0.05)} style={section}>
        ...
        <div style={grid4}>
          ...
          <StatCard
            ...
            color={
              data && data.kyc.dropoff.dropoff_rate_pct > 10
                ? "#ef4444"
                : "#f59e0b"
            }
          />
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: "240px" }}>
              ...
              {[
                {
                  label: "Verified",
                  count: data.users.kyc.verified,
                  total: data.users.total_registered,
                  color: "#10b981",
                },
                {
                  label: "Pending",
                  count: data.users.kyc.pending,
                  total: data.users.total_registered,
                  color: "#f59e0b",
                },
                {
                  label: "Rejected",
                  count: data.users.kyc.rejected,
                  total: data.users.total_registered,
                  color: "#ef4444",
                },
                {
                  label: "Unverified",
                  count: data.users.kyc.unverified,
                  total: data.users.total_registered,
                  color: "#94a3b8",
                },
              ].map((b) => {
                const pct =
                  data.users.total_registered > 0
                    ? (b.count / data.users.total_registered) * 100
                    : 0;
                return (
                  <div key={b.label} style={{ marginBottom: "10px" }}>
                    ...
                    >
                      <span>{b.label}</span>
                      <span style={{ color: "#0A1E28", fontWeight: 700 }}>
      ...
      <motion.div {...card(0.1)} style={section}>
        ...
        {data && (
          ...
          >
            <div>
              ...
              >
                {formatNgn(data.trades.lifetime_ngn_volume)}
              </div>
            </div>
            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
              <div style={{ textAlign: "center" }}>
                ...
                >
                  {formatNgn(data.trades.avg_trade_size_ngn)}
                </div>
              </div>
              <div style={{ textAlign: "center" }}>
                <div
                ...
                >
                  {formatHours(data.trades.avg_settlement_hours)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={grid4}>
          <StatCard
            ...
            value={data?.trades.total_initiated.toLocaleString() ?? "—"}
            icon={<Activity size={16} />}
            color="#3b82f6"
          />
          <StatCard
            label="Completed"
            value={data?.trades.total_completed.toLocaleString() ?? "—"}
            icon={<CheckCircle size={16} />}
            color="#10b981"
          />
          <StatCard
            label="Cancelled"
            value={data?.trades.total_cancelled.toLocaleString() ?? "—"}
            icon={<AlertCircle size={16} />}
            color="#f59e0b"
          />
          <StatCard
            label="Disputed"
            value={data?.trades.total_disputed.toLocaleString() ?? "—"}
            icon={<AlertCircle size={16} />}
            color="#ef4444"
          />
        </div>
        ...
        {data && (
          ...
          >
            ...
            >
              <CircularGauge
                pct={data.trades.success_rate_pct}
                label="Success Rate"
                color="#10b981"
              />
              <div
                  ...
                  <tbody>
                    {data.trades.by_currency.map((row) => (
                      ...
                      >
                        ...
                        >
                          {row.completed_trades.toLocaleString()}
                        </td>
                        <td
                          style={{
                            padding: "10px 12px",
                        ...
                        >
                          {formatNgn(row.lifetime_ngn_volume)}
                        </td>
                        <td style={{ padding: "10px 12px", color: "#64748b" }}>
                          {formatNgn(row.avg_trade_size_ngn)}
                        </td>
                        <td style={{ padding: "10px 12px", color: "#64748b" }}>
                          {formatHours(row.avg_settlement_hours)}
                        </td>
                      </tr>
                    ))}
      ...
      <motion.div {...card(0.15)} style={section}>
        ...
        <div style={grid4}>
          <StatCard
            ...
            value={data?.p2p.active_listings.toLocaleString() ?? "—"}
            sub="Currently live"
            icon={<Activity size={16} />}
            color="#f59e0b"
          />
          <StatCard
            label="Total Sellers"
            value={data?.p2p.total_sellers.toLocaleString() ?? "—"}
            icon={<Users size={16} />}
            color="#3b82f6"
          />
          <StatCard
            label="Total Buyers"
            value={data?.p2p.total_buyers.toLocaleString() ?? "—"}
            icon={<Users size={16} />}
            color="#8b5cf6"
          />
          <StatCard
            label="Repeat Traders"
            value={data?.p2p.repeat_traders.toLocaleString() ?? "—"}
            sub={
              data
                ? `${formatPct(data.p2p.repeat_rate_pct)} repeat rate`
                : undefined
            }
        ...
        {data && (
          ...
          >
            ...
            >
              <div
                style={{
                  ...
                  textTransform: "uppercase",
                  letterSpacing: "0.05em",
                  color: disputeRateColor(data.p2p.dispute_rate_pct),
                  marginBottom: "4px",
                }}
              >
                Dispute Rate
              ...
              >
                {formatPct(data.p2p.dispute_rate_pct)}
              </div>
              <div
            ...
            <div style={{ flex: 2, minWidth: "220px" }}>
              ...
              >
                Disputes: {data.p2p.disputes.total}
              </div>
              {[
                {
                  label: "Open",
                  count: data.p2p.disputes.open,
                  color: "#ef4444",
                },
                {
                  label: "Resolved",
                  count: data.p2p.disputes.resolved,
                  color: "#10b981",
                },
              ].map((d) => {
                const pct =
                  data.p2p.disputes.total > 0
                    ? (d.count / data.p2p.disputes.total) * 100
                    : 0;
                return (
      ...
      <motion.div {...card(0.2)} style={section}>
        ...
        {data && (
          <div
            style={{
              background: "linear-gradient(135deg, #0A1E28, #1a3344)",
              borderRadius: "16px",
              padding: "20px 24px",
              marginBottom: "20px",
              display: "flex",
          ...
          >
            ...
            <div style={{ display: "flex", gap: "32px", flexWrap: "wrap" }}>
              <div>
                ...
                >
                  {formatNgn(data.escrow.lifetime_volume_ngn)}
                </div>
                <div
                  style={{
                    fontSize: "10px",
              ...
              <div>
                ...
                >
                  {formatNgn(data.escrow.current_volume_ngn)}
                </div>
                <div
                  style={{
                    fontSize: "10px",
            ...
            >
              <div style={{ textAlign: "center" }}>
                ...
                >
                  {data.escrow.currently_escrowed_trades}
                </div>
              ...
              <CircularGauge
                pct={data.escrow.completion_rate_pct}
                label="Completion"
                color="#C8F032"
                size={72}
              />
              ...
              {data.escrow.live_tvl_by_currency.map((e) => (
                <div
                  ...
                  style={{
                    padding: "16px",
                    borderRadius: "12px",
                    background: `${currencyColor[e.currency]}10`,
                    border: `1px solid ${currencyColor[e.currency]}30`,
                    textAlign: "center",
                  }}
                >
                  ...
                  >
                    {Number(e.locked_value).toLocaleString("en-NG", {
            ...
            <tbody>
              {(data?.deposits.by_currency ?? []).map((row) => (
                ...
                >
                  ...
                  >
                    {row.total_deposits.toLocaleString()}
                  </td>
                  <td
                    style={{
                      padding: "12px 16px",
                  ...
                  >
                    {row.completed_deposits.toLocaleString()}
                  </td>
                  <td
                    style={{
                      padding: "12px 16px",
                  ...
                  >
                    {row.pending_deposits.toLocaleString()}
                  </td>
                  <td
                    style={{
                      padding: "12px 16px",
                  ...
                  >
                    {row.rejected_deposits.toLocaleString()}
                  </td>
                  <td
                    style={{
                      padding: "12px 16px",
                  ...
                  >
                    {formatVolume(row.total_volume, row.currency)}
                  </td>
                </tr>
              ))}
      ...
      <motion.div {...card(0.3)} style={section}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "40px" }}>
          <div>
            ...
            >
              {growthFilter === "7d"
                ? "New registrations per day with cumulative overlay"
                : growthFilter === "4w"
                  ? "New registrations per week with cumulative overlay"
                  : growthFilter === "12m"
                    ? "New registrations per month with cumulative total overlay"
                    : "All-time monthly registrations from Oct 2023 with cumulative overlay"}
            </div>
            <BarChart
          ...
          <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: "32px" }}>
            ...
            >
              {growthFilter === "7d"
                ? "Completed trades per day with cumulative NGN volume"
                : growthFilter === "4w"
                  ? "Completed trades per week with cumulative NGN volume"
                  : growthFilter === "12m"
                    ? "Completed trades per month with cumulative NGN volume"
                    : "All-time monthly trade volume from Oct 2023 (cumulative)"}
            </div>
            <BarChart
        ...
        <motion.div {...card(0.35)} style={{ ...section, marginBottom: 0 }}>
          ...
          >
            {[
              {
                label: "PIN Window",
                value: `${data.platform.pin_confirmation_window_minutes} min`,
              },
              {
                label: "Bid Expiry",
                value: `${data.platform.bid_expiry_hours} hrs`,
              },
              {
                label: "Background Jobs",
                value: String(data.platform.background_jobs_count),
              },
              {
                label: "KYC Process",
                value: data.platform.kyc_process.replace(/_/g, " "),
              },
              {
                label: "NGN Deposit",
                value: data.platform.deposit_ngn_provider,
              },
              {
                label: "Foreign Deposit",
                value: data.platform.deposit_foreign_process.replace(/_/g, " "),
              },
            ].map((item) => (
              <div
                key={item.label}
          ...
          >
            ...
            {data.platform.supported_currencies.map((c) => (
