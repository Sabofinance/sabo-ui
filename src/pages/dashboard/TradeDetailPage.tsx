...
const TradeDetailPage: React.FC = () => {
  ...
  const [pinAction, setPinAction] = useState<'sellerConfirm' | 'complete' | null>(null);
  ...
  const fetchTrade = useCallback(async (isInitial = false) => {
    if (isInitial) setLoading(true);
    try {
      const res = await tradesApi.getById(tradeId);
      if (res.success && res.data) {
        setTrade(res.data as TradeData);
        if ((res.data as TradeData).status === 'completed' || (res.data as TradeData).status === 'cancelled') {
  ...
  const executePinAction = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(pinAction === 'sellerConfirm' ? 'Trade confirmed!' : 'Trade completed successfully!');
        ...
        else if (msg.includes('PIN_NOT_SET')) toast.error('Please set your Transaction PIN first.');
        else if (msg.includes('INSUFFICIENT_BALANCE')) toast.error('Insufficient wallet balance.');
  ...
  const handleSubmitRating = async () => {
    ...
    setProcessing(true);
    try {
      const res = await ratingsApi.create({ trade_id: tradeId, score: ratingScore, comment: ratingComment });
      if (res.success) {
  ...
  if (loading) return <div className="dashboard-wrapper"><main className="transaction-page"><p>Loading trade details...</p></main></div>;
  if (error || !trade) return <div className="dashboard-wrapper"><main className="transaction-page"><p>{error || 'Trade not found.'}</p></main></div>;
  ...
  return (
    <div className="dashboard-wrapper">
      <main className="transaction-page">
        <button className="back-btn" onClick={() => navigate('/dashboard/trades')}>
        ...
        <div className="transaction-header">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <p>ID: <strong>{trade.id}</strong> • Status: <strong style={{ color: getStatusColor(trade.status), textTransform: 'capitalize' }}>{trade.status}</strong></p>
            </div>
            {trade.status !== 'completed' && trade.status !== 'cancelled' && (
              <button className="page-number" style={{ color: '#EF4444', borderColor: '#EF4444' }} onClick={() => setShowDisputeModal(true)}>
                Raise Dispute
              </button>
            )}
          </div>
          ...
          <div className="exchange-card">
            <div className="display-group">
              <div>
                ...
                <div className="locked-number">{trade.amount.toLocaleString()}</div>
              </div>
            ...
            <div className="visual-bridge">
              <div className="bridge-line"><div className="bridge-node node-top"></div><div className="bridge-node node-bottom"></div></div>
            </div>

            <div className="display-group">
              <div>
                <span className="display-label">TOTAL (NGN)</span>
                <div className="locked-number">{trade.total.toLocaleString()}</div>
              </div>
              <div className="flag-section">
            ...
            <div className="rate-footer">
              ...
              <span className="rate-footer-value">1 {trade.currency} = {trade.rate.toLocaleString()} NGN</span>
            </div>
          </div>
          ...
          <div className="info-sidebar">
            ...
            <div className="tile" style={{ background: '#F8FAFC' }}>
              ...
              {trade.status === 'escrowed' && (
                <div>
                  {isSeller ? (
                    <>
                      <p style={{ marginBottom: 20 }}>Please confirm this trade within the window.</p>
                    ...
                    <p>Waiting for seller to confirm the trade. Please hold on...</p>
                  )}
                </div>
              )}
              ...
              {trade.status === 'completed' && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <button className="btn-market" style={{ marginTop: 20 }} onClick={() => setShowRatingModal(true)}>Rate Counterparty</button>
                </div>
              )}
              ...
              {trade.dispute_status && (
                <div style={{ marginTop: 20, padding: 12, background: '#FEE2E2', borderRadius: 12, border: '1px solid #FECACA', color: '#991B1B', fontWeight: 700 }}>
            ...
            <div className="tile">
              ...
              <div className="merchant-row">
                <img src={isSeller ? (trade.buyer_avatar || 'https://via.placeholder.com/56') : (trade.seller_avatar || 'https://via.placeholder.com/56')} className="merchant-avatar" alt="avatar" />
                <div>
                  <div className="merchant-name">{isSeller ? trade.buyer_name : trade.seller_name}</div>
                  <div className="merchant-meta">{isSeller ? 'Buyer' : 'Seller'}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {showPinModal && (
        <div className="modal-overlay" onClick={() => setShowPinModal(false)}>
          <div className="confirm-modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <h2>Enter PIN</h2>
            <p style={{ color: '#64748B', marginBottom: 24 }}>Enter your 6-digit transaction PIN to authorize this action.</p>
            <PinDotsInput value={pin} onChange={setPin} disabled={processing} autoFocus />
            <div className="modal-actions" style={{ marginTop: 30 }}>
              <button className="cancel-btn" onClick={() => setShowPinModal(false)}>Cancel</button>
              <button className="confirm-btn" onClick={executePinAction} disabled={processing || pin.length < 6}>
      ...
      {showDisputeModal && (
        <div className="modal-overlay" onClick={() => setShowDisputeModal(false)}>
          <div className="confirm-modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close-icon" onClick={() => setShowDisputeModal(false)}>×</button>
            <h2>Raise Dispute</h2>
            <p style={{ color: '#64748B', marginBottom: 16 }}>Describe the issue with this trade. Minimum 20 characters.</p>
            <textarea
              className="dispute-textarea"
              style={{ width: '100%', minHeight: 120, borderRadius: 12, border: '1px solid #E2E8F0', padding: 12, marginBottom: 20 }}
              value={disputeReason}
              onChange={e => setDisputeReason(e.target.value)}
              placeholder="Provide details..."
            />
            <div className="modal-actions">
              <button className="cancel-btn" onClick={() => setShowDisputeModal(false)}>Cancel</button>
              <button className="confirm-btn" style={{ background: '#EF4444' }} onClick={handleRaiseDispute} disabled={processing || disputeReason.length < 20}>
      ...
      {showRatingModal && (
        <div className="modal-overlay" onClick={() => setShowRatingModal(false)}>
          <div className="success-modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close-icon" onClick={() => setShowRatingModal(false)}>×</button>
            <h2 className="success-title">Rate Experience</h2>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', margin: '20px 0' }}>
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} onClick={() => setRatingScore(n)} style={{ fontSize: 24, background: 'none', border: 'none', cursor: 'pointer', color: ratingScore >= n ? '#C8F032' : '#E2E8F0' }}>★</button>
              ))}
            </div>
            <textarea
              style={{ width: '100%', minHeight: 80, borderRadius: 12, border: '1px solid #E2E8F0', padding: 12, marginBottom: 20 }}
              value={ratingComment}
              onChange={e => setRatingComment(e.target.value)}
              placeholder="Optional comment..."
            />
            <button className="btn-market" onClick={handleSubmitRating} disabled={processing}>
